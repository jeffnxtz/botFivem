  ...
  exec(interaction) {
    if (interaction.type === Discord.InteractionType.ApplicationCommand) {
      ...
      if (interaction.customId === "whitelistModal") {
        ...
        connect.query(
          `SELECT * FROM accounts WHERE id = '${characterId}'`,
          async (error, rows) => {
            if (error) {
              console.error(error);
            ...
            connect.query(
              `SELECT * FROM accounts WHERE id = '${characterId}' AND whitelist = 1`,
              async (error, rows) => {
                if (error) {
                  console.error(error);
                  return interaction.reply({
                ...
                // Aprova whitelist
                connect.query(
                  `UPDATE accounts SET whitelist = '1' WHERE id = '${characterId}'`,
                  async (error) => {
                    if (error) {
                      console.error(error);
                      return interaction.reply({
